...
export declare type RequestedRoonServices = RoonApiBrowse | RoonApiImage | RoonApiTransport;
export declare type ProvidedRoonServices = RoonApiStatus | object;
export interface RoonServiceOptions {
    required_services?: {
        new (): RequestedRoonServices;
    }[];
    optional_services?: {
        new (): RequestedRoonServices;
    }[];
...
export declare type RoonSubscriptionResponse = 'Subscribed' | 'Changed' | 'Unsubscribed';
...
export declare type RoonApiBrowseHierarchy = 'browse' | 'playlists' | 'settings' | 'internet_radio' | 'albums' | 'artists' | 'genres' | 'composers' | 'search';
...
export declare type RoonImageScale = 'fit' | 'fill' | 'stretch';
export declare type RoonImageFormat = 'image/jpeg' | 'image/png';
export interface RoonApiImage {
    get_image(image_key: string, options: RoonApiImageResultOptions): Promise<{
        content_type: string;
        image: Buffer;
    }>;
}
...
export declare type RoonPlaybackState = 'playing' | 'paused' | 'loading' | 'stopped';
export interface ZoneSettings {
    loop: ZoneLoopSettings;
    shuffle: boolean;
    auto_radio: boolean;
}
export declare type ZoneLoopSettings = 'loop' | 'loop_one' | 'disabled';
...
export declare type OutputSourceControlStatus = 'selected' | 'deselected' | 'standby' | 'indeterminate';
...
export declare type OutputVolumeControlType = 'number' | 'db' | 'incremental';
export interface RoonApiTransportSettings {
    shuffle?: boolean;
    auto_radio?: boolean;
    loop?: RoonLoopOptions;
}
export declare type RoonLoopOptions = 'loop' | 'loop_one' | 'disabled' | 'next';
export declare type RoonChangeVolumeHow = 'absolute' | 'relative' | 'relative_step';
export declare type RoonApiTransportControl = 'play' | 'pause' | 'playpause' | 'stop' | 'previous' | 'next';
export interface RoonApiTransportConvenienceSwitchOptions {
    control_key?: string;
}
export declare type RoonMuteHow = 'mute' | 'unmute';
export declare type RoonSeekHow = 'relative' | 'absolute';
export interface RoonApiTransportStandbyOptions {
    control_key?: string;
}
export declare type RoonApiTransportOutputSubscriptionCallback = (response: RoonSubscriptionResponse, body: RoonApiTransportOutputs) => void;
export interface RoonApiTransportOutputs {
    outputs: Output[];
}
export declare type RoonApiTransportZonesSubscriptionCallback = (response: RoonSubscriptionResponse, body: RoonApiTransportZones) => void;
export interface RoonApiTransportZones {
    zones?: Zone[];
    zones_changed?: Zone[];
    zones_seek_changed?: Pick<Zone, 'zone_id' | 'queue_time_remaining' | 'seek_position'>[];
}
export declare type RoonApiTransportQueueSubscriptionCallback = (response: RoonSubscriptionResponse, body: RoonApiTransportQueue) => void;
