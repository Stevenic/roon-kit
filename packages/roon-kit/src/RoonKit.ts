...
export class RoonKit {
     ...
     */
    public static readonly RoonApi: new (options: RoonApiOptions) => RoonApi = require('node-roon-api');
     ...
     */
    public static readonly RoonApiStatus: new (roon: RoonApi) => RoonApiStatus = require('node-roon-api-status');
            ...
            switch (key) {
                ...
                case 'core_lost':
                    ...
                    if (typeof cb == 'function') {
                        options[key] = (core: RoonCore) => cb(proxyCore(core));
                    }
                    break;
            }
        } 
...
type CoreCallback = (core: RoonCore) => void;

interface RoonCoreProxy extends RoonCore {
    isProxy?: boolean;
}

function proxyCore(core: RoonCoreProxy): RoonCore {
    if (!core.isProxy) {
        // Proxy services
        if (core.services?.RoonApiBrowse) {
            (core.services as any).RoonApiBrowse = proxyBrowse(core.services.RoonApiBrowse);
        }

        if (core.services?.RoonApiImage) {
            (core.services as any).RoonApiImage = proxyImage(core.services.RoonApiImage);
        }

        if (core.services?.RoonApiTransport) {
            (core.services as any).RoonApiTransport = proxyTransport(core.services.RoonApiTransport);
        }
        ...
        get(t, p, r) {
            ...
            switch (p) {
                case 'get_image':
                    ...
                    v = (...args: any[]) => {
                        return new Promise((resolve, reject) => {
                            args.push((err: string | false, content_type: string, image: Buffer) => {
                                if (err) {
                                    reject(err);
                                } else {
